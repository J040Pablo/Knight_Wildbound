...
namespace Roguelite.Environment
{
    public class WorldBoundary : MonoBehaviour
    {
        ...
        public bool IsPositionInsideBoundary(Vector3 position, float margin = 0.5f)
        {
            ...
            Vector3 min = worldBounds.min + new Vector3(margin, -10f, margin);
            Vector3 max = worldBounds.max - new Vector3(margin, -10f, margin);
        ...
        private void CreatePerimeterWalls()
        {
            ...
            // 1. North Wall (+Z)
            CreateWall("InvisibleWall_North", new Vector3(center.x, center.y, center.z + size.z / 2f + wallThickness / 2f), new Vector3(size.x + wallThickness * 2f, wallHeight, wallThickness));
            // 2. South Wall (-Z)
            CreateWall("InvisibleWall_South", new Vector3(center.x, center.y, center.z - size.z / 2f - wallThickness / 2f), new Vector3(size.x + wallThickness * 2f, wallHeight, wallThickness));
            // 3. East Wall (+X)
            CreateWall("InvisibleWall_East", new Vector3(center.x + size.x / 2f + wallThickness / 2f, center.y, center.z), new Vector3(wallThickness, wallHeight, size.z));
            // 4. West Wall (-X)
            CreateWall("InvisibleWall_West", new Vector3(center.x - size.x / 2f - wallThickness / 2f, center.y, center.z), new Vector3(wallThickness, wallHeight, size.z));
        }

        private GameObject CreateWall(string wallName, Vector3 wallPos, Vector3 wallSize)
        {
